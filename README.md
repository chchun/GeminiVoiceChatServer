...
현재 **Stage 1 (MVP)** 구현 단계: 텍스트 스트리밍 완료, KPI TTFT < 1.0 s.
...
```
Android Client
     │  WebSocket  ws://host:port/ws?api_key=<KEY>
     ▼
┌─────────────────────────────┐
│  FastAPI (uvicorn)          │
│  ┌──────────────────────┐   │
...
# WebSocket 인증 키 (Android 클라이언트와 동일한 값 사용)
...
**엔드포인트:** `ws://[host]:[port]/ws?api_key=<WS_API_KEY>`
...
// 모드 전환 (ACK 없음, 상태만 업데이트)
{"type": "mode_change", "mode": "TEXT" | "VOICE"}

// [Stage 2] 오디오 입력 — Stage 1에서는 error 응답
{"type": "audio_input", "payload": "<base64>", "is_final": true}
```
...
| `AUDIO_FORMAT_INVALID` | 오디오 포맷 오류 (Stage 1: audio_input 수신 시) |
| `GEMINI_API_ERROR` | Gemini API 호출 실패 |
| `INTERNAL_ERROR` | 서버 내부 오류 |

에러 발생 시 `error` + `status(ERROR)` 순서로 전송하고 **세션을 유지**합니다.

---

## 스모크 테스트

실행 중인 서버에 대해 5개 시나리오를 자동으로 검증합니다:

```powershell
# 기본 (localhost:8000, 모든 테스트)
python scripts/ws_smoke.py

# 서버 주소 지정
python scripts/ws_smoke.py --host 192.168.1.100 --port 8000
...
| # | 테스트 | 검증 내용 |
|---|--------|-----------|
| 1 | bad_auth | 잘못된 api_key → 연결 거부 (close 1008) |
| 2 | ready | 올바른 api_key → `status(READY)` 수신 |
| 3 | ping_pong | `ping` 전송 → `pong` 수신 |
| 4 | text_input_flow | 스트리밍 흐름 전체 (`THINKING` → chunks → `text_done`) |
| 5 | multi_turn | 2번째 턴에서 1번째 턴의 컨텍스트 유지 확인 |

---

## 레이턴시 하네스

각 대화 턴마다 4개의 타임스탬프를 캡처하여 구조화된 JSON 로그로 출력합니다:

| 키 | 측정 시점 |
|----|-----------|
| `ts_server_recv` | 클라이언트 메시지 수신 |
| `ts_llm_req` | Gemini API 호출 시작 |
| `ts_llm_ttft` | Gemini 첫 번째 토큰 수신 |
| `ts_server_send_end` | `text_done` 패킷 전송 완료 |
...
```json
{
  "event": "turn_latency",
  "trace_id": "a1b2c3d4-...",
  "ts_server_recv": 1716000000000,
  "ts_llm_req": 1716000000005,
  "ts_llm_ttft": 1716000000320,
  "ts_server_send_end": 1716000002100,
  "ttft_ms": 320,
  "total_ms": 2100
}
```
...
| Stage | 기능 | 상태 |
|-------|------|------|
| **1** | FastAPI WebSocket + Gemini 텍스트 스트리밍 | **완료** |
| 2 | PCM 오디오 입력 + STT 파이프라인 | 예정 |
| 3 | LangGraph + Chroma DB 벡터 RAG | 예정 |
| 4 | 멀티 에이전트 라우터 (법률 검색, 고객 지원) | 예정 |
